...
pub enum Error {
    ...
    #[error(
        "apply stopped while replaying branch `{branch}` commit `{commit}` in worktree {worktree}: {message}\n\nResolve the conflicts in that worktree, stage the resolved files with `git -C {worktree} add <files>`, then run `git cascade continue`. Do not run `git cherry-pick --continue` or `git merge --continue` manually; git-cascade will finish the operation after checking its recovery state."
    )]
    ApplyStopped {
        branch: String,
        commit: String,
